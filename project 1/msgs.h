...
struct q_node * createItem(int *msg, struct queue *client_port) {
	struct q_node * new_node = (struct q_node *) malloc(sizeof(struct q_node));
	for(int i=0; i<10; i++)
		new_node->data.msg[i]=*(msg+i);
...
void send_msg(struct queue * port, int *msg, struct queue *cport) {
	struct q_node * node = createItem(msg, cport);
	return addItemToPortQueue(port, node);
}
